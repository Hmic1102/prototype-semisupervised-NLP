...
def test_accu(num,model):
  ...
  test_dataset = DBpedia(".torchtext/cache", split='test')
...
# Load dataset
test_dataset = DBpedia(root=".torchtext/cache", split='test')
test_data = [(label, text) for label, text in test_dataset]
random.shuffle(test_data)
...
student = BertForSequenceClassification.from_pretrained('bert-base-uncased', num_labels=14).to(device)
...
train_dataset = DBpedia(".torchtext/cache", split='train')
